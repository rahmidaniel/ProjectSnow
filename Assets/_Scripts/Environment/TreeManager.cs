...
namespace _Scripts.Environment
{
    ...
    public class TreeManager : TimeListener, IPersistentData
    {
        ...
        {
            if (data.treePositions.Count == 0)
            {
                SpawnInArea();
                return;
            }

            if (_trees.Count != 0) _trees.ForEach(Destroy);
        ...
        private void SpawnInArea()
        {
            ...
            foreach (var tree in
                     _points.Select(point =>
                         Instantiate(treePrefab, point,
                             Quaternion.identity))) _trees.Add(tree); // for saving and loading
        }
        ...
        private Vector2 PlaceOnGround(float x, float fromY)
        {
            ...
            Debug.DrawLine(new Vector3(x, fromY), hit.point, Color.red, 10, false);
            var newY = hit.point.y + _treeCollider.offset.y + _treeCollider.size.y; // moving to ground position
        ...
        private void CalculateTreePosition(Vector2 transformPosition, float sizeX)
        {
            ...
            if (TreeFits(leftSizeX)) CalculateTreePosition(transformPosition, leftSizeX);

            var rightMin = newX + treeOffsetX;
            var rightSizeX = right - rightMin;
            if (TreeFits(rightSizeX)) CalculateTreePosition(new Vector2(rightMin, transformPosition.y), rightSizeX);
        }
